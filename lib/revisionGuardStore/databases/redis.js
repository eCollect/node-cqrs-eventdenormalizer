...
_.extend(Redis.prototype, {
  ...
  _getAsync: async function (id) {
    ...
    const entryParsed = jsondate.parse(entry.toString());
    return entryParsed.revision || null;
  },
  ...
  _saveLastEventAsync: async function (evt) {
    ...
    return this.client.set(key, JSON.stringify({ event: evt }));
  },
